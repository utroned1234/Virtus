...
export default function BuyPackagePage() {
  ...
  // Trigger a verification immediately after submitting
  useEffect(() => {
    if (!purchaseId) return
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#34D399] text-sm">{t('buy.loading')}</p>
        </div>
      </div>
    )
  }
          ...
          <div className="glass-card !p-8 text-center space-y-5 w-full">
            ...
            <div>
              <h2 className="text-xl font-bold text-white mb-2">
                {isUpgrade ? t('buy.upgradeActivated') : t('buy.packageActivated')}
              </h2>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#34D399]/15 border border-[#34D399]/30">
                <div className="w-2 h-2 rounded-full bg-[#34D399]" />
                <span className="text-sm font-semibold text-[#34D399]">{t('buy.active')}</span>
              </div>
            </div>

            <p className="text-white/60 text-sm leading-relaxed">
              {isUpgrade
                ? t('buy.upgradeSuccess').replace('{{name}}', pkg.name)
                : t('buy.packageSuccess').replace('{{name}}', pkg.name)}
              {' '}{t('buy.dailyProfitActive').replace('{{amount}}', String(pkg.daily_profit_bs))}
            </p>
      ...
      <div className="min-h-screen pb-20">
        <div className="max-w-md mx-auto p-6 flex flex-col items-center justify-center min-h-[70vh]">
          <div className="glass-card !p-8 text-center space-y-5 w-full">
            ...
            <div>
              <h2 className="text-xl font-bold text-white mb-2">{t('buy.paymentNotVerified')}</h2>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-red-500/15 border border-red-500/30">
                <span className="text-sm font-semibold text-red-400">{t('buy.rejected')}</span>
              </div>
            </div>
      ...
      <div className="min-h-screen pb-20">
        <div className="max-w-md mx-auto p-6 flex flex-col items-center justify-center min-h-[70vh]">
          <div className="glass-card !p-8 text-center space-y-5 w-full">
            ...
            <h2 className="text-lg font-bold text-white">{t('buy.verifying')}</h2>
            ...
            <p className="text-xs text-white/60">
              {t('buy.confirmations')
                .replace('{{count}}', String(confirmations))
                .replace('{{total}}', String(requiredConfs))}
            </p>
      ...
      <div className="max-w-md mx-auto p-4 space-y-4">
        ...
        <div className="text-center pt-2 pb-1">
          ...
          <p className="text-xs text-white/50 mt-1 uppercase tracking-wider">
            {isUpgrade ? t('buy.upgradePlan') : t('buy.completeYourPurchase')}
          </p>
        </div>
        ...
        <div className="glass-card !p-4 space-y-2">
          {isUpgrade && activePurchase && (
            <div className="flex justify-between items-center text-xs text-white/50 pb-2 border-b border-white/10">
              <span>{t('buy.currentPlan')}: {activePurchase.vip_package.name}</span>
              <span>${activePurchase.investment_bs.toLocaleString()} USD</span>
            </div>
          )}
          <div className="flex justify-between items-center">
            <span className="text-xs text-white/50 uppercase tracking-wider">
              {isUpgrade ? t('buy.newPlan') : t('buy.investment')}
            </span>
            <span className="text-lg font-bold text-[#FBBF24]">${pkg.investment_bs.toLocaleString()} USD</span>
          </div>
          {isUpgrade && (
            <div className="flex justify-between items-center">
              <span className="text-xs text-white/50 uppercase tracking-wider">{t('buy.payNow')}</span>
              <span className="text-lg font-bold text-[#34D399]">+${amountToPay.toLocaleString()} USD</span>
            </div>
          )}
          <div className="flex justify-between items-center">
            <span className="text-xs text-white/50 uppercase tracking-wider">{t('buy.dailyProfit')}</span>
